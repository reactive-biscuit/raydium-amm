...
pub struct AmmInfoHack {
    ...
    /// All fee information
    pub fees: crate::state::Fees,
    /// Statistical data
    pub state_data: StateDataHack,
    /// Coin vault
...
impl_loadable!(AmmInfoHack);

#[repr(C)]
#[derive(Clone, Copy, Debug, Default, PartialEq)]
pub struct StateDataHack {
...
impl From<StateDataHack> for crate::state::StateData {
    fn from(value: StateDataHack) -> Self {
        Self {
...
impl From<AmmInfoHack> for crate::state::AmmInfo {
    fn from(value: AmmInfoHack) -> Self {
        crate::state::AmmInfo {
            status: value.status,
            nonce: value.nonce,
